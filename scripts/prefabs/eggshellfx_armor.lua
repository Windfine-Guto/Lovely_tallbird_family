local assets =
{
    Asset("ANIM", "anim/fx_armor_eggshell.zip"),
}
...
local function OnUpdateThorns(inst)
    ...
    local armor = inst.owner and inst.owner.components.inventory and
    inst.owner.components.inventory:GetEquippedItem(EQUIPSLOTS.BODY) or nil
    inst.damage = armor and armor.components.armor and
    armor.components.armor:GetPercent() * 70 or 70
...
local function MakeFX(name)
    local function fn()
        ...
        inst.AnimState:SetBank("fx_armor_eggshell")
        inst.AnimState:SetBuild("fx_armor_eggshell")
        inst.AnimState:PlayAnimation("idle")

        inst:SetPrefabNameOverride("fx_armor_eggshell")
...
return MakeFX("eggshellfx_armor")
