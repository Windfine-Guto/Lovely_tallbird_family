local assets =
{
    Asset("ANIM", "anim/tallbird_flute.zip"),
    Asset("IMAGE","images/inventoryimages/tallbird_flute.tex"),
	Asset("ATLAS", "images/inventoryimages/tallbird_flute.xml"),
    Asset("IMAGE","images/inventoryimages/tallbird_flute_work.tex"),
	Asset("ATLAS", "images/inventoryimages/tallbird_flute_work.xml"),
}
...
local function OnPlayed(inst, musician)
    if inst:HasTag("cannotsleep") then
        return
    end
...
local function HearPanFlute(inst, musician, instrument)
    if instrument:HasTag("cannotsleep") then
        return
    end
    if inst ~= musician and
        ...
        and inst.components.combat and inst.components.combat.target==musician then
...
local function OnDropped(inst)
    if inst:HasTag("bird_store") then
        ...
        inst.AnimState:PlayAnimation("idle_work", false)
    end
end
...
local function Fn()
    ...
    inst.AnimState:SetBank("tallbird_flute")
    inst.AnimState:SetBuild("tallbird_flute")
    inst.AnimState:PlayAnimation("float", false)
    ...
    inst.components.inventoryitem.imagename = "tallbird_flute"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/tallbird_flute.xml"
    ...
    inst.components.finiteuses:SetOnFinished(function (inst)
        inst:AddTag("cannotsleep")
    end)
    inst.components.finiteuses:SetConsumption(ACTIONS.BIRD_STORE, 1)
    ...
    inst:ListenForEvent("floater_startfloating", function(inst)
        if inst:HasTag("bird_store") then
            ...
            inst.AnimState:PlayAnimation("float_work", false)
        end
    end)
...
return Prefab("tallbird_flute",Fn, assets,prefabs)
